...
docker tag eschool-backend gcr.io/google_project_name/eschool-backend:0.0.1
gcloud auth activate-service-account --key-file /tmp/ansible/.ssh/service_account.json
gcloud docker -- push gcr.io/google_project_name/eschool-backend
gcloud beta container clusters get-credentials eschool-claster --region us-central1 --project google_project_name
kubectl create secret docker-registry gcr-json-key --docker-server=gcr.io --docker-username=_json_key --docker-password="$(cat /tmp/ansible/.ssh/gcp-viewer.json)" --docker-email=yaroslavmd@gmail.com
